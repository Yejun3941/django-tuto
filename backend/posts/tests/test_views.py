...
class PostAPITest(TestCase):
    def setUp(self):
        ...
        self.token_url = reverse("token_obtain_pair") # "token_obtain_pair"는 DRF JWT에서 제공하는 토큰 발급 URL
        # 또는 JWT로 로그인 후 토큰을 헤더에 세팅하는 방법도 가능

        # Obtain JWT token
        response = self.client.post(self.token_url, {'username': 'testuser', 'password': '1234abcd'}, format='json')
